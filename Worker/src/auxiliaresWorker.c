...
int conectarYCargar(int nquant, t_list *nodos, int **fds, char ***lns) {
    ...
    for (i = 0; i < nquant; ++i) {
        ...
        msj = serializar_FName(n->fname, &len);
        head.sizeData = len;

        msg = createMessage(&head, msj);

        enviar_message(*fds[i], msg, logw, &ctl);
...
int apareoGlobal(t_list *nodos, char *fname) {
    ...
    //remaining++; //todo: ver que onda

    //todo: chequear para funcionamiento sin nodos servidores
    ...
    while (remaining) {
        ...
        // Pedimos siguiente linea del menor string
        head.letra = 'W';
        head.codigo = 13;
        head.sizeData = 1;
        msg = createMessage(&head, "");
        enviar_message(fds[min], msg, logw, &ctl);
