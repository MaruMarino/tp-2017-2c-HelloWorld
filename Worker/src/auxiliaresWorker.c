...
int crearArchivoBin(char *bin, size_t bin_sz, char *fname) {
    ...
    // todo: cambiamos permisos a 777 de 055
...
int crearArchivoData(size_t blk, size_t count, char *fname) {
    ...
    //todo: comentamos free, segfault ... y pareciera que no va, puntero área mapeada
//	free(data);
    fclose(f);
    return 0;
}
...
int realizarApareo(int nfiles, FILE *fs[nfiles]) {
    //todo: no llega bien la lista de punteros de FIlE, rompe con seg fault en la linea 152 cuando haces fgets() :)
...
int makeCommandAndExecute(char *data_fname, char *exe_fname, char *out_fname) {
    ...
    // todo: entraba por error cuando es success , porque el exitstatus de system era 0
    int algo = system(cmd);
    if (algo != 0) { // ejecucion del comando via system()
