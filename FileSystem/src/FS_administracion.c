...
int recuperar_estructuras_administrativas(void) {
	...
	if (status != 1) {
		log_error(logi,
				"Error en la recuperacion del arbol de directorios, bai");
		return status;
	}
	// recuperar NODOS
	...
	if (status != 1) {
		log_error(logi,
				"Error en la recuperacion de metadata de archivos, bai");
		return status;
	}
...
int recuperar_nodos(void) {
	...
	configuracion->espacio_total = (config_get_int_value(config_nodos,
			"TAMANIO")) * Mib;
...
int recuperar_bitmap_nodo(NODO *unodo) {
	...
	char *aux_nombre_archivo = string_from_format("bitmaps/%s.dat",
			unodo->nombre);
	...
	char *bitarray = mmap(0, mystat.st_size, PROT_READ | PROT_WRITE, MAP_SHARED,
			fdbitmap, 0);
	...
	unodo->bitmapNodo = bitarray_create_with_mode(bitarray, mystat.st_size,
			LSB_FIRST);
	...
	while (aux_split[i] != NULL) {
		...
		if (archivoss == NULL) {
			...
			while (aux_split_archs[c] != NULL) {
				fullpath_archivo = string_from_format("%s/%s", fullpath,
						aux_split_archs[c]);
...
int recuperar_metadata_un_arhcivo(char *fullpath, int padre) {
	...
	while (cantidad_leidas < limite) {
		...
		nuevobloque->bytesEnBloque = config_get_int_value(info,
				key_bloque_bytes);
		cantidad_leidas++;
...
int iniciar_arbol_directorios(void) {
	...
	memcpy(directorios[0].nombre, configuracion->dir_estructuras,
			strlen(configuracion->dir_estructuras));
...
int iniciar_bitmaps_nodos(void) {
	...
	void _create_bitmap(NODO *self) {
		...
		self->bitmapNodo = bitarray_create_with_mode(bitarray,
				(size_t) cantBloques, LSB_FIRST);
	}
...
int existe_dir_en_padre(char *nombre, int padre) {
	int i;
	for (i = 0; i < 100; i++) {
		if (!strcmp(directorios[i].nombre, nombre)
...
void crear_metadata_archivo(t_archivo *arch) {
	...
	char *path_armado_conNombre = string_from_format("%s/%s", path_armado,
			arch->nombre);
	...
	void _agregar_info_bloques(bloqueArchivo *self) {
		...
		if (self->bloquenodo0 != -1) {
			...
			copia0 = string_from_format("[%s,%d]", self->nodo0,
					self->bloquenodo0);
		...
		if (self->bloquenodo1 != -1) {
			...
			copia1 = string_from_format("[%s,%d]", self->nodo1,
					self->bloquenodo1);
...
static char *nombres_subdirectorios(char *donde) {
	...
	for (entry = readdir(directorio); entry != NULL;
			entry = readdir(directorio)) {

		if (((strncmp(entry->d_name, ".", 1))
				|| (strncmp(entry->d_name, "..", 1))) && entry->d_type == DT_DIR) {
...
static char *nombres_archivos(char *donde) {
	...
	for (entry = readdir(directorio); entry != NULL;
		...
		if (((strncmp(entry->d_name, ".", 1))
				|| (strncmp(entry->d_name, "..", 1))) && entry->d_type == DT_REG) {
